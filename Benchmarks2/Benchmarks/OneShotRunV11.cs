...
public class OneShotRunV11
{
    ...
    public void Setup()
    {
        ...
        while (root != null && !Directory.Exists(Path.Combine(root, "testapp11")))
        {
        ...
        testappFolder = Path.Combine(root, "testapp11");
        Console.WriteLine($"testapp folder: '{testappFolder}'");

        jitApp = Path.Combine(testappFolder, @"net11.0\TestAot2.exe");
        ...
        aotBaseApp = Path.Combine(testappFolder, @"base11\TestAot2-base11.exe");
        aotNativeApp = Path.Combine(testappFolder, @"native11\TestAot2-native11.exe");
